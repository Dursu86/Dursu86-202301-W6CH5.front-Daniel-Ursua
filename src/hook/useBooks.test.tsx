...
describe("Given the useCharacters hook", () => {
  ...
  let mockRepo: BooksRepo;
  let mockErrorRepo: BooksRepo;
  const spyOn = jest.spyOn(console, "error");

  const mockBooks: BookStructure[] = [
    {
      id: 1,
      name: "John D. Dorian",
      occupattion: "doctor",
  ...
  const mockScrubPartial: Partial<BookStructure> = { id: 1, name: "Test ok" };

  const mockStore = configureStore({
    reducer: { scrubs: bookReducer },
    preloadedState: {
      scrubs: {
        scrubs: [
          {
